...
The Microarray object contains several levels of microarray associated data, organised in a (fairly) intuitive way. First, there's the data that you have obtained from a microarray scanner, in the form of a data file. This is imported into Microrray as a L<Data_File|TRL::Microarray::Microarray_File::Data_File> object. Support for different data file formats is built into the L<Data_File|TRL::Microarray::Microarray_File::Data_File> class, and creating new classes for your favourite scanner/software output is relatively simple. Data extracted from the microarray spots are then imported into individual L<array_spot|TRL::Microarray::Spot> objects. Next, replicate spots are collated into L<array_feature|TRL::Microarray::Feature> objects. Most of the quality control functions operating on parameters such as signal intensity and spot size, are built into this final process, so that an L<array_feature|TRL::Microarray::Feature> object only contains data from spots that have passed the QC assessments. 
...
The data file can be passed to Microarray either as a file name, filehandle object, or L<data_file|TRL::Microarray::Microarray_File::Data_File> object. If a filehandle is passed, the filename also needs to be set. 
...
L<TRL::Microarray::Microarray_File|TRL::Microarray::Microarray_File>, L<TRL::Microarray::Feature|TRL::Microarray::Feature>, L<TRL::Microarray::Spot|TRL::Microarray::Spot>
